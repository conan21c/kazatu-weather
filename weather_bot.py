...
def get_weather_desc(code):
    # WMO Weather interpretation codes (WMO) - EXHAUSTIVE MAP
    weather_codes = {
        0: ("맑음", "☀️"), 
        1: ("구름 조금", "🌤️"), 2: ("구름 많음", "⛅"), 3: ("흐림", "☁️"),
        45: ("안개", "🌫️"), 48: ("안개", "🌫️"),
        51: ("가벼운 이슬비", "🌧️"), 53: ("이슬비", "🌧️"), 55: ("짙은 이슬비", "🌧️"),
        56: ("가벼운 얼음이슬비", "🌨️"), 57: ("짙은 얼음이슬비", "🌨️"),
        61: ("가벼운 비", "☂️"), 63: ("보통 비", "☔"), 65: ("강한 비", "🌧️"),
        66: ("가벼운 얼음비", "🌨️"), 67: ("짙은 얼음비", "🌨️"),
        71: ("가벼운 눈", "🌨️"), 73: ("보통 눈", "❄️"), 75: ("강한 눈", "❄️"),
        77: ("눈발", "❄️"), 
        80: ("가벼운 소나기", "🚿"), 81: ("보통 소나기", "🌧️"), 82: ("강한 소나기", "⛈️"),
        85: ("가벼운 소낙눈", "🌨️"), 86: ("강한 소낙눈", "❄️"),
        95: ("천둥번개", "⛈️"), 96: ("천둥번개(가벼운 우박)", "⛈️"), 99: ("천둥번개(강한 우박)", "⛈️")
    }
    # 만약 코드가 없어도 ? 대신 근사한 아이콘으로 대체
...
def generate_ai_comment(weather_data):
    if not GEMINI_API_KEY:
        return "내일 투어 시 지역별 고도차에 따른 기온 변화에 유의하시고, 즐거운 여행 되세요!"
    ...
    try:
        ...
        내일 투어 날씨 데이터: {data_str}
        
        [지시사항]
        1. 뻔한 인사말이나 장황한 설명은 절대 금지합니다.
        2. 주로 '낮 시간대(Daytime)' 투어 활동에 직접적으로 영향을 주는 기상 변화만 언급하세요.
        3. 실전적인 핵심 조언 2~3줄로만 작성하세요. (최대한 짧고 임팩트 있게)
        4. 낮에 비나 눈이 올 경우, 혹은 고산 지대의 강한 자외선이나 바람 등 '활동'에 방해되는 요소를 짚어주고 해결책(준비물)을 제시하세요.
        5. 아씨고원을 제외한 투어는 당일치기임을 인지하고 답변하세요.
        
        예시: "내일 낮 차른캐년은 해가 뜨거우니 선글라스가 필수이며, 아씨고원은 눈발이 날릴 수 있으니 4륜 구동 차량과 두꺼운 외투를 반드시 준비하세요."
        """
